...
void print_device_properties(void)
{
    ...
    printf("CUDA devices visible to this rank: %d\n", dev_count);

    for (int i = 0; i < dev_count; i++) {
        cudaDeviceProp prop;
        cudaGetDeviceProperties(&prop, i);
        printf("  Device %d: %s (sm_%d%d, %d SMs, %.1f GB, %.1f GB/s peak)\n",
                  ...
                  int **row_indices, int **col_indices, float **values)
{
    ...
    int is_symmetric = mm_is_symmetric(matcode) || mm_is_hermitian(matcode) || mm_is_skew(matcode);
    int is_pattern = mm_is_pattern(matcode);

    /* symmetric storage lists only half the entries: reserve room for the mirror */
    ...
    float *vals = (float *)malloc((size_t)max_nnz * sizeof(float));
                ...
                int **row_ptr_out, int **col_idx_out, float **csr_values_out)
{
    ...
    int *ci = (int *)malloc((size_t)nnz * sizeof(int));
    float *cv = (float *)malloc((size_t)nnz * sizeof(float));
